<?php
	...
	$sizeQuery = mysqli_query($link, 'SELECT * FROM `size` WHERE (`id` < 35);');
	...
	$subCategoryQuery = mysqli_query($link, 'SELECT * FROM `footwear_category`;');
...
<div class="categoryMenuConteiner">
	<div data-show="floor" :class="{ categoryMenuButtonClick: categoryMenu.floor.show }" class="categoryMenuButton" v-on:click="categoryMenuOpen">Пол:</div>
		<div class="categoryMenu" v-if="categoryMenu.floor.show" :class="{ categoryMenuOpen: categoryMenu.floor.opacity }">
			...
			<div class="categorySelectElem df aic" data-id="0" @click="categorySelectElem('floor')">Универсальный</div>
		</div>
	</div>
	<div class="categoryMenuConteiner">
...
<div class="categoryMenuConteiner">
	...
	<div class="categoryMenu df jc fw" v-if="categoryMenu.size.show" :class="{ categoryMenuOpen: categoryMenu.size.opacity }">
	<?php
	...
	?>
		...
		<div class="categorySelectElem df jc aic categorySelectSize" data-id="<?php echo $size['id'] ?>" @click="categorySelectElem('size')"><?php echo $size['value'] ?></div>
	...
	<div class="categoryMenuConteiner">
		...
		?>
			...
			<div v-if="categoryMenu.views_sport.show" class="categorySelectElem df aic" data-id="<?php echo $sport['id'] ?>" @click="categorySelectElem('views_sport')"><?php echo $sport['name'] ?></div>
...
<div class="categoryMenuConteiner">
	<div data-show="subCategory" :class="{ categoryMenuButtonClick: categoryMenu.subCategory.show }" class="categoryMenuButton" v-on:click="categoryMenuOpen">Вид обуви:</div>
	<div class="categoryMenu" v-if="categoryMenu.subCategory.show" :class="{ categoryMenuOpen: categoryMenu.subCategory.opacity }">
		...
		?>
			...
			<div class="categorySelectElem df aic" data-id="<?php echo $subCategory['id']; ?>" @click="categorySelectElem('subCategory')"><?php echo $subCategory['name']; ?></div>
...
<div class="categoryMenuConteiner">
	...
	<div class="categoryMenu" v-if="categoryMenu.time_year.show" :class="{ categoryMenuOpen: categoryMenu.time_year.opacity }">
	<?php
	...
	?>
		...
		<div class="categorySelectElem df aic" data-id="<?php echo $season['id'] ?>" @click="categorySelectElem('time_year')"><?php echo $season['name'] ?></div>
